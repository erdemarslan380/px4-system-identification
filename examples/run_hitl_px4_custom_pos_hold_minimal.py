...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description='Minimal baseline to custom_pos_control handover test.')
    ...
    p.add_argument('--direct-z-tolerance', type=float, default=0.20)
    ...
    *,
    ...
    while time.time() < end:
        ...
        if now >= next_report and latest_lpos and latest_att:
            print(f"{label} x={float(latest_lpos.x):.3f} y={float(latest_lpos.y):.3f} z={float(latest_lpos.z):.3f} vx={float(latest_lpos.vx):.3f} vy={float(latest_lpos.vy):.3f} vz={float(latest_lpos.vz):.3f} roll={math.degrees(float(latest_att.roll)):.2f} pitch={math.degrees(float(latest_att.pitch)):.2f}", flush=True)
    ...
    try:
        ...
        if ground_xy > args.ground_xy_window or abs(baseline_z) > args.ground_z_window:
            raise RuntimeError(
                ...
                "Restart HIL clean before running custom_pos handover."
            )
        ...
        print(f'Custom handover target x={target_x:.3f} y={target_y:.3f} z={target_z:.3f} yaw={math.degrees(yaw0):.1f}deg', flush=True)
        ...
        latest_lpos, latest_att, hold_max_xy, hold_max_z_err, hold_max_tilt, hold_failsafe = observe_phase(
            mav,
            label='Custom hold',
            duration_s=args.hold_seconds,
            report_period=args.report_period,
            ref_x=custom_ref_x,
            ref_y=custom_ref_y,
            ref_z=custom_ref_z,
        ...
        final_xy = math.hypot(float(latest_lpos.x) - custom_ref_x, float(latest_lpos.y) - custom_ref_y)
        final_z_err = abs(float(latest_lpos.z) - custom_ref_z)
        final_tilt = math.degrees(max(abs(float(latest_att.roll)), abs(float(latest_att.pitch))))
        print(
            f'Custom hold summary: max_xy={hold_max_xy:.3f}m max_z_err={hold_max_z_err:.3f}m '
            f'max_tilt={hold_max_tilt:.2f}deg failsafe={hold_failsafe}',
            flush=True,
        )

        if (
            hold_failsafe
            or hold_max_xy > args.xy_limit
            or hold_max_z_err > args.custom_z_tolerance
            or final_xy > args.xy_limit
            or final_z_err > args.z_tolerance
            or hold_max_tilt > args.tilt_limit_deg
            or final_tilt > args.tilt_limit_deg
        ):
            raise RuntimeError(
                f'Custom hold unstable: max_xy={hold_max_xy:.3f}m final_xy={final_xy:.3f}m '
                f'max_z_err={hold_max_z_err:.3f}m final_z_err={final_z_err:.3f}m '
                f'max_tilt={hold_max_tilt:.2f}deg final_tilt={final_tilt:.2f}deg failsafe={hold_failsafe}'
            )

        set_param(mav, 'CST_POS_CTRL_EN', 0, mavutil.mavlink.MAV_PARAM_TYPE_INT32)
        mav.mav.set_mode_send(mav.target_system, mavutil.mavlink.MAV_MODE_FLAG_CUSTOM_MODE_ENABLED, 3 << 16)
        disarm(mav)
        print('Custom hold stable', flush=True)
        return 0
    finally:
        try:
            set_param(mav, 'CST_POS_CTRL_EN', 0, mavutil.mavlink.MAV_PARAM_TYPE_INT32)
