...
- Runs built-in `identification_only`, `trajectory_only`, and `full_stack` campaigns while keeping the return-to-anchor legs out of the CSV logs.
- Keeps the same campaign names across SITL, HIL, and real-flight workflows so the operator can reuse one documented procedure.
